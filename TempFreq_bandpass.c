...
static void caerHelloWorldModuleRun(caerModuleData moduleData, size_t argsNumber, va_list args){
	...
	if(state->init == false){
		...
		for(size_t coreid=1; coreid<4; coreid++){
			...
			caerDynapseSetBias(stateSource, DYNAPSE_CONFIG_DYNAPSE_U0, coreid, "NPDPIE_TAU_F_P", 4, 41, "HighBias","PBias");
		...
		for(size_t neuronid = 0; neuronid<6; neuronid++){
			for (size_t camid = 0; camid<2; camid++){
				...
				caerDynapseWriteCam(stateSource->deviceState, 8, neuronid, camid, DYNAPSE_CONFIG_CAMTYPE_F_EXC);
				caerDynapseWriteCam(stateSource->deviceState, 8, neuronid+16, camid, DYNAPSE_CONFIG_CAMTYPE_F_EXC);
				caerDynapseWriteCam(stateSource->deviceState, 9, neuronid+16, camid+2, DYNAPSE_CONFIG_CAMTYPE_F_EXC);
				caerDynapseWriteCam(stateSource->deviceState, 9, neuronid+16*2, camid, DYNAPSE_CONFIG_CAMTYPE_F_EXC);
				caerDynapseWriteCam(stateSource->deviceState, 10, neuronid+16*2, camid+2, DYNAPSE_CONFIG_CAMTYPE_F_EXC);
				caerDynapseWriteCam(stateSource->deviceState, 10, neuronid+16*3, camid, DYNAPSE_CONFIG_CAMTYPE_F_EXC);
		...
		for(size_t preneuron = 0; preneuron<6; preneuron++){
			caerDynapseWriteCam(stateSource->deviceState, preneuron, 529, camid, DYNAPSE_CONFIG_CAMTYPE_S_EXC);//17 core 2
			caerDynapseWriteCam(stateSource->deviceState, preneuron+16, 529, camid+1, DYNAPSE_CONFIG_CAMTYPE_S_EXC);
			caerDynapseWriteCam(stateSource->deviceState, preneuron+16*2, 529, camid+2, DYNAPSE_CONFIG_CAMTYPE_S_INH);
			caerDynapseWriteCam(stateSource->deviceState, preneuron+16*3, 529, camid+3, DYNAPSE_CONFIG_CAMTYPE_S_INH);

			caerDynapseWriteCam(stateSource->deviceState, preneuron, 273, camid, DYNAPSE_CONFIG_CAMTYPE_S_EXC);//17 core 1
			caerDynapseWriteCam(stateSource->deviceState, preneuron+16, 273, camid+1, DYNAPSE_CONFIG_CAMTYPE_S_EXC);
			caerDynapseWriteCam(stateSource->deviceState, preneuron+16*2, 273, camid+2, DYNAPSE_CONFIG_CAMTYPE_S_INH);
			caerDynapseWriteCam(stateSource->deviceState, preneuron+16*3, 273, camid+3, DYNAPSE_CONFIG_CAMTYPE_S_INH);
